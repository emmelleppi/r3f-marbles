...
function usePostprocessing() {
  ...
  const [composer] = useMemo(() => {
    ...
    const renderEffectPass = new EffectPass(camera, new GammaCorrectionEffect({ gamma: 0.5 }))
    renderEffectPass.encodeOutput = false // Prevent potential bugs.
    const backfaceEffectPass = new EffectPass(refCameraLayer2.current, new GammaCorrectionEffect({ gamma: 0.5 }))
    backfaceEffectPass.encodeOutput = false // Prevent potential bugs.
    const renderEnvEffectPass = new EffectPass(refCameraLayer1.current, new GammaCorrectionEffect({ gamma: 0.5 }))
